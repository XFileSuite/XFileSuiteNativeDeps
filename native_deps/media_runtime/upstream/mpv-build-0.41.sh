...
sed -i '' \
    ...
    meson.build
cp "${PROJECT_DIR}/scripts/mpv/app-bridge-no-swift.m" \
    osdep/mac/app_bridge_no_swift.m
sed -i '' \
    "s#'osdep/mac/app_bridge.m'#'osdep/mac/app_bridge.m', 'osdep/mac/app_bridge_no_swift.m'#" \
    meson.build
